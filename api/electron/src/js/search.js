...
function searchByDieDate(){
  ...
  if(month==''){
    ...
    fetch('http://localhost:4848/api/search/secondSearchByDateDie', {
            method: 'POST',
            headers: {
              'Accept': 'application/json',
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({day:day,month:month})
          }).then((res)=>{
            return res.json()

          }).then((data)=>{
            if(data.searchedShahid.length>0){console.log('im more than 0')}
            else{createToast('error','در این تاریخ شهیدی یافت نشد')}
            console.log('____________>',data)
            data.searchedShahid.forEach((element ,index)=> {
                console.log(element.name);
                const cardWraper=document.getElementById('card-wraper')
